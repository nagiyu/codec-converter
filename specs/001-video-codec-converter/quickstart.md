...
S3 をユーザーから隠すために、S3 オブジェクトをクライアントへストリーム転送し、`Content-Disposition: attachment` を設定する Next.js の API ルートを実装します。ガイダンス:

- `GET /api/download?jobId=<id>` または `GET /api/download/<jobId>` を実装し、以下を行います:
...
export default async function handler(req, res) {
	// jobId の検証、DynamoDB の確認などを行う
	const s3Resp = await s3.send(new GetObjectCommand({ Bucket, Key }));
	res.setHeader('Content-Type', s3Resp.ContentType || 'application/octet-stream');
